...
│   └── shim.html                 สะพาน google.script.run -> /api/rpc
│
...
- ส่งซ้ำได้ด้วยฟังก์ชัน `inviteEmployee(empId)`
...
**ยกเว้นรายคน** — เครื่องที่ไม่รองรับ ใช้ `setWebauthnExempt(empId, true)`
(เก็บที่ `profiles.webauthnExempt`) พนักงานคนนั้นจะเช็คอินได้โดยไม่ต้องใช้ passkey
...
รันชุดทดสอบ **90 ข้อ** บนฐานข้อมูลจำลองในหน่วยความจำที่โหลด **ข้อมูลจริงจาก TapTime.xlsx**
