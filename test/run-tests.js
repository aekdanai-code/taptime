...
(async () => {
  ...
  await t('คู่มือ: มีไฟล์ทั้งฝั่งแอดมินและพนักงาน + เปิดเดี่ยว ๆ ได้', () => {
    for (const f of ['manual-admin.html', 'manual-employee.html']) {
      const h = readDoc(f);
      assert.ok(h.startsWith('<!DOCTYPE html>'), f + ' ไม่ใช่ HTML ที่สมบูรณ์');
      assert.ok(/<html lang="th">/.test(h), f + ' ไม่ได้ระบุภาษาไทย');
      assert.ok(h.includes('<meta name="viewport"'), f + ' ไม่มี viewport (เปิดบนมือถือจะเพี้ยน)');
      assert.ok(h.includes('@media print'), f + ' พิมพ์เป็น PDF แล้วหน้าตาจะพัง');
      // ต้องเปิดจากไฟล์ได้โดยไม่พึ่งเซิร์ฟเวอร์ — ห้ามอ้างไฟล์ในโปรเจกต์
      assert.ok(!/(src|href)="\/(?!\/)/.test(h), f + ' อ้างไฟล์ด้วย path จากรากเว็บ');
      assert.ok(!/<script src=/.test(h), f + ' ดึงสคริปต์ภายนอก');
    }
  });

  await t('คู่มือ: ข้อความในคู่มือตรงกับปุ่ม/ข้อความจริงในระบบ', () => {
    const emp = fs.readFileSync(path.join(GEN, 'employee.html'), 'utf8');
    const adm = fs.readFileSync(path.join(GEN, 'admin.html'), 'utf8');

    // สิ่งที่คู่มือพนักงานบอกว่าจะเห็น ต้องมีอยู่จริงในหน้าพนักงาน
    ['ยื่นขอลา', 'ยื่นขอทำ OT', 'ขอแก้ไขเวลา', 'ขออนุมัติทำงานวันหยุด',
     'เช็คอินซ้ำ', 'เช็คเอาท์ซ้ำ', 'ยังไม่ได้เช็คอิน', 'เข้าถึงตำแหน่งไม่ได้',
     'ตำแหน่งไม่แม่นยำพอ', 'ไม่ได้รับตำแหน่ง', 'เวลาทำงานปกติ']
      .forEach((k) => assert.ok(emp.includes(k), 'คู่มือพนักงานอ้างถึง "' + k + '" แต่ระบบไม่มี'));

    // สิ่งที่คู่มือแอดมินบอกว่าจะเห็น ต้องมีอยู่จริงในหน้าแอดมิน
    ['ตั้งค่า OT', 'อนุมัติ OT', 'รายงาน OT', 'ส่งอีเมลเชิญใหม่', 'ทดลองคำนวณ',
     'เช็คชื่อแทน (เข้า)', 'เช็คชื่อแทน (ออก)', 'คีย์ OT', 'อนุมัติที่เลือก',
     'คัดลอกตาราง', 'ให้สิทธิ์พนักงานทุกคน', 'หักเวลาพักเมื่อทำงานถึง']
      .forEach((k) => assert.ok(adm.includes(k), 'คู่มือแอดมินอ้างถึง "' + k + '" แต่ระบบไม่มี'));
  });

  await t('คู่มือ: ระบุข้อจำกัดที่ต้องบอกผู้ใช้ให้รู้ตัว', () => {
    const a = readDoc('manual-admin.html');
    const e = readDoc('manual-employee.html');
  ...
  await t('คู่มือ: อธิบายสูตรตรงกับที่โค้ดคำนวณจริง', () => {
    const a = readDoc('manual-admin.html');
    // ลำดับ 5 ชั้นต้องเรียงถูก (ดูเฉพาะในตารางกฎ ไม่ใช่ทั้งไฟล์ เพราะสารบัญมีคำเดียวกัน)
    const rules = a.slice(a.indexOf('id="ot-rule"'), a.indexOf('id="ot-cap"'));
    const order = ['ช่วงผ่อนผัน', 'หักพักระหว่าง OT', 'ปัดเศษ', 'ขั้นต่ำต่อครั้ง', 'เพดาน']
      .map((s) => rules.indexOf(s));
    order.forEach((i, n) => assert.ok(i > 0, 'ไม่พบชั้นที่ ' + (n + 1)));
    for (let i = 1; i < order.length; i++)
      assert.ok(order[i] > order[i - 1], 'ลำดับกฎในคู่มือสลับกับของจริง');

    // ข้อที่เข้าใจผิดง่ายที่สุด ต้องเขียนไว้ให้ชัด
    assert.ok(a.includes('ตัดทั้งก้อน'), 'คู่มือไม่ได้อธิบายว่าขั้นต่ำตัดทั้งก้อน');
    assert.ok(/ต้อง.{0,4}"เกิน"/.test(a), 'คู่มือไม่ได้อธิบายว่าผ่อนผันต้อง "เกิน"');
    assert.ok(a.includes('จันทร์ถึงอาทิตย์') || a.includes('จันทร์–อาทิตย์'),
      'คู่มือไม่ได้บอกว่าสัปดาห์เริ่มวันจันทร์');
    assert.ok(a.includes('ชั่วโมงงานปกติ + ชั่วโมง OT'),
      'คู่มือไม่ได้อธิบายว่าชั่วโมงงานกับ OT ไม่นับซ้ำ');
    // ตัวเลขในตัวอย่างต้องคำนวณได้จริงตามสูตร
    const r = overtime.computeOt({
      checkInMin: 480, checkOutMin: helpers.toMinutes('19:20'),
      workStartMin: 480, workEndMin: 1020, dayType: 'workday',
      policy: overtime.normalizePolicy({
        mode: 'auto', graceMinutes: 15, minMinutes: 30,
        roundMinutes: 30, roundMode: 'down', maxPerDayMinutes: 240,
      }),
    });
    assert.strictEqual(r.raw, 140, 'ตัวอย่างในคู่มือ: นาทีดิบไม่ตรง');
    assert.strictEqual(r.countable, 120, 'ตัวอย่างในคู่มือ: ผลลัพธ์ไม่ตรง');
  });
